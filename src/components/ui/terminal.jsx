...
export function Terminal({
  ...
  // Added "restarting" phase for loop clear + restart
  ...
  useEffect(() => {
    ...
    // Wait loopDelay, then wipe all state and start over
    const t = setTimeout(() => {
      setLines([]);
      setCurrentText("");
      setCommandIdx(0);
      setCharIdx(0);
      setOutputIdx(-1);
      setPhase("typing"); // skip "idle" – we're already in view
    }, loopDelay);

    return () => clearTimeout(t);
  }, [phase, loop, loopDelay]);
  // ──────────────────────────────────────────────────────────────────────────
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-xl px-2 sm:px-4 font-mono text-[10px] sm:text-xs",
        className,
      )}
    >
      <div className="overflow-hidden rounded-lg border border-neutral-800 bg-[#0a0a0a] shadow-2xl">
        {/* Title Bar */}
        <div className="flex items-center gap-1.5 sm:gap-2 bg-[#131313] px-3 sm:px-4 py-2 sm:py-3">
          ...
          <div className="flex-1 text-center">
            <span className="truncate text-[10px] sm:text-xs text-neutral-400">
              {username} — bash
            </span>
          </div>
          <div className="w-[36px] sm:w-[52px]" />
        </div>

        {/* Terminal Content */}
        <div
          ref={contentRef}
          className="no-visible-scrollbar scrollbar-hide h-56 sm:h-80 overflow-y-auto p-3 sm:p-4 font-mono"
        >
          {lines.map((line, i) => (
